...
public class ValidatorHelper {
    ...
    public static boolean checkSimultaneousViolationsOrNegation(RecreationModel mergedKB,
        ...
        for (RecreationModel sourceModel : sourceModels) {
            if (!sourceModel.getConstraints().isEmpty()) {
                testModel.addConstraint(new OrNegationConstraint(sourceModel.getConstraints()));
            }
        }

        // Convert to Choco model and check satisfiability
        ChocoModel chocoModel = ChocoTranslator.convertToChocoModel(testModel);
        return Analyser.isConsistent(chocoModel);
    }
    ...
    /**
     * Implementation of Test Case 2 for a specific original knowledge base against multiple other knowledge bases.
     * 
     * Tests the formula: ¬KBMerge ∧ originalKB with region constraints:
     * - Forces the tested region to true
     * - Forces all other regions to false
     * - Forces unknown features (not in originalKB) to false
     * 
     * This ensures that every valid configuration from the original knowledge
     * base is preserved in the merged result when operating in that region.
     ...
     * @param mergedKB             the merged knowledge base to test against
     * @param originalKB           the original knowledge base being verified
     * @param otherKBs             array of other original knowledge bases (for region exclusion)
     * @return true if missing solutions exist (validation fails),
     *         false if all original solutions preserved (validation passes)
     */
    public static boolean checkMissingSolutionsMultiple(RecreationModel mergedKB, RecreationModel originalKB,
            RecreationModel[] otherKBs) {
        logger.info("\t[checkMissingSolutionsMultiple] Checking for missing solutions in {}", originalKB.getRegionString());
        ...
        // REGION ISOLATION: Force the tested region to true
        String regionName = originalKB.getRegion().getRegionString();
        FeatureReferenceConstraint fRef = new FeatureReferenceConstraint();
        fRef.setFeature(testModel.getFeatures().get(regionName));
        testModel.addConstraint(fRef); // tested region = true

        // Force all other regions to false
        for (RecreationModel otherKB : otherKBs) {
            String otherRegionName = otherKB.getRegion().getRegionString();
            NotConstraint notConstraint = new NotConstraint();
            FeatureReferenceConstraint otherFRef = new FeatureReferenceConstraint();
            otherFRef.setFeature(testModel.getFeatures().get(otherRegionName));
            notConstraint.setInner(otherFRef);
            testModel.addConstraint(notConstraint); // other region = false
            logger.trace("\t[checkMissingSolutionsMultiple] forcing region {} to false", otherRegionName);
        }
        ...
        // Identify features that exist in the merged model but not in the current region
        // These represent "foreign" features that should be forced to false during testing
        Set<String> forbidden = new HashSet<>(mergedKB.getFeatures().keySet()); // all merged vars
        forbidden.removeAll(originalKB.getFeatures().keySet()); // keep only unknowns
        forbidden.remove(originalKB.getRegion().getRegionString()); // never forbid tested region marker
        
        // Remove all other region markers from forbidden set
        for (RecreationModel otherKB : otherKBs) {
            forbidden.remove(otherKB.getRegion().getRegionString());
        }
        ...
        for (String uniqueFeatureName : forbidden) {
            ...
            if (uniqueFeature != null) {
                ...
                forceFalseConstraint.setInner(featureRef);
                testModel.addConstraint(forceFalseConstraint);
                logger.trace("\t[checkMissingSolutionsMultiple] forcing unique feature {} to false", uniqueFeatureName);
            }
        }

        // TEST FORMULA: ¬KBMerge ∧ originalKB
        // Add ¬KBMerge using OrNegationConstraint (at least one merged constraint violated)
        testModel.addConstraint(new OrNegationConstraint(mergedKB.getConstraints()));
        ...
        if (isSatisfiable) {
            logger.warn(
                    "\t[checkMissingSolutionsMultiple] Test Case 2 for {} FAILED: KBMerge excludes valid configurations (merge too strict)",
                    originalKB.getRegionString());
            return true;
        } else {
            logger.info(
                    "\t[checkMissingSolutionsMultiple] Test Case 2 for {} PASSED: KBMerge includes all valid configurations",
                    originalKB.getRegionString());
            return false;
        }
    }
