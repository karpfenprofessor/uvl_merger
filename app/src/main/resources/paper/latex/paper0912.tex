...
To complement the automotive case study, we conducted an additional
evaluation using a set of real-world variability models from the
financial services domain.  The models were obtained from a publicly
available UVL repository~\cite{uvlhub2024} and represent independently
maintained variants of a large configuration knowledge base.  Each
model contains more than 700 features and over 2\,000 cross-tree
constraints, making them substantially larger than the models used in
the previous experiment.

\subsection{Models}

Nine UVL models were selected for analysis.  While the models share a
common vocabulary, they differ in feature composition, constraint
density and structural organization.  Table~\ref{tab:finance_stats}
summarizes their size.  Models~2--5 exhibit particularly high
similarity, whereas models~6--10 form a second subgroup with stronger
internal variation and lower similarity to the first group.
...
Figure~\ref{fig:finance-two-plots} provides an overview of the two key
dimensions investigated: (a) reduction of constraint density, and (b)
the relationship between feature overlap and contextualization in the
resulting merged models.
...
For highly similar model pairs, more than half of the cross-tree
constraints are removed, indicating strong redundancy between the
variants.  Less similar pairs exhibit a weaker but still substantial
reduction, demonstrating that large-scale real-world models contain
significant semantic overlap.

The contextualization plot highlights three regions.  Model pairs with
high structural similarity often require only minimal contextualization.
In contrast, pairs with similar feature sets but diverging constraint
semantics show a markedly higher contextualization share.  Pairs with
low feature overlap exhibit only moderate contextualization, as many
constraints remain local to one of the original variants.

\subsection{Findings}

The evaluation on industrial-scale models demonstrates the following:

\begin{itemize}[leftmargin=*]
\item Substantial redundancy is present even in large, independently
      developed models.
\item Contextualization increases when constraint semantics diverge,
      not when feature overlap decreases.
\item Low-overlap merges remain manageable, as many constraints do not
      introduce inter-model interactions.
\end{itemize}

Overall, the experiment shows that the consistency-based merging
approach scales to large real-world variability models and
successfully balances redundancy elimination with preservation of
